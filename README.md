...
[**VIEWPORT.BREAK.1.0.2.dmg をダウンロード**](https://github.com/nakatadesign/VIEWPORT-BREAK/releases/download/v1.0.2/VIEWPORT.BREAK.1.0.2.dmg)（約 599 KB / macOS 12 以降）

ad-hoc 署名のみで Developer ID 署名・公証は通していないため、初回起動時だけ Gatekeeper の警告が出ます。
`Applications` に入れたアプリを **右クリック →［開く］** で起動し、出たダイアログでもう一度［開く］を選べば、次回からは普通にダブルクリックで開けます。

DMG の中身と詳しい手順は [インストール](#インストール)、過去の版は
[Releases](https://github.com/nakatadesign/VIEWPORT-BREAK/releases) にあります。

### 自分でビルドする（開発者向け）

macOS 12 以降と `swiftc` が要ります。

```bash
./packaging/build_dmg.sh     # → build/VIEWPORT BREAK 1.0.2.dmg
```
